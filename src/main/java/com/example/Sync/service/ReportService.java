...
public class ReportService {
    ...
    private double getAdjustmentTotal(Long orderId) {

        Order order = orderRepo.findById(orderId)
                .orElse(null);

        if (order == null ||
                order.getAdjustments() == null) {
            return 0;
        }

        return order.getAdjustments()
                .stream()
                .mapToDouble(OrderAdjustment::getAmount)
                .sum();
    }
    ...
    public Map<String, Object> getDailyReport(
        ...
        List<Object[]> rows =
                orderRepo.findSettledOrdersBetween(
                        start,
                        end
                );
        ...
        for (Object[] row : rows) {
            ...
            double adjustmentTotal =
                    getAdjustmentTotal(orderId);

            double total =
                    originalTotal - adjustmentTotal;
    ...
    public Map<String, Object> getTopProducts(
        ...
        List<Object[]> rows =
                orderRepo.findAllSettledOrders();
...
// ────────────────────────────────────────────────────────────────
    ...
    public Map<String, Object> getCustomReport(
        ...
        for (Object[] row : rows) {
            ...
            boolean payMatch =
                    payment == null
                            || payment.equalsIgnoreCase("all")
                            || pay.equalsIgnoreCase(payment);

            boolean typeMatch =
                    orderType == null
                            || orderType.equalsIgnoreCase("all")
                            || type.equalsIgnoreCase(orderType);
